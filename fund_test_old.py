...
def simulation(sell_share_prop, out_of_cost_range, operate_freq, days=730):
    """
    ...
    for i in range(400):
        ...
        for index, price in sample_sr.items():
            if price<=anchor*0.96:  # 比锚点下跌4%，买进
        ...
        profit_rates.append(round((hold_amount-cost+history_profit)/max_cost*100, 4) if max_cost!=0 else 0)
...
if __name__ == "__main__":
    ...
    # df = file.parse('查询1')
    df = get_history_prices(code, '2018-09-02', '2021-11-30')
    df.set_index('date', inplace=True)
    sr = df['price']
    sr.sort_index(inplace=True)
    # print(sr)

    # simulation(0.3, 0.15, 20, 180)

    a1,a2=0,0
    max_mean=0
    list=[]
    for i in [0.15, 0.2, 0.3, 0.4, 0.5]:
        for k in [10, 20, 30, 40]:
            # print('sell_share_prop: {}, out_of_cost_range: {}, operate_freq: {}'.format(i,j,k))
            mean=simulation(i, 0.15, k, 180)
            if mean>max_mean:
                a1, a2, max_mean=i, k, mean
    

    print('sell_share_prop: {}, operate_freq: {}, max_mean: {}'.format(a1,a2,max_mean))
